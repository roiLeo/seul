...
function getUniquesClassThawedData(
  ctx: EventHandlerContext<Store>
): IUniquesClassId {
  const event = new events.UniquesClassThawedEvent(ctx)
  if (event.isV1) {
...
export const uniqueClassThawed = (
  ctx: EventHandlerContext<Store>
): Promise<void> => processClassStateChange(ctx, getUniquesClassThawedData, Status.ACTIVE, EventType.THAWE)
