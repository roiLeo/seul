...
interface IUniquesThawedData {
  classId: number
  instanceId: number
}

function getUniquesThawedData(
  ctx: EventHandlerContext<Store>
): IUniquesThawedData {
  const event = new events.UniquesThawedEvent(ctx)
  if (event.isV1) {
...
export async function uniqueThawed(
  ctx: EventHandlerContext<Store>
): Promise<void> {
  const eventData = getUniquesThawedData(ctx)
