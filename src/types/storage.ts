...
export class UniquesAccountStorage {
   ...
   *  account can be enumerated.
   */
  async getAsV3(key1: Uint8Array, key2: number, key3: number): Promise<null | undefined> {
    assert(this.isV3)
    return this._chain.getStorage(this.blockHash, 'Uniques', 'Account', key1, key2, key3)
  }

  async getManyAsV3(keys: [Uint8Array, number, number][]): Promise<(null | undefined)[]> {
    assert(this.isV3)
...
export class UniquesClassStorage {
   ...
   *  Details of an asset class.
   */
  async getAsV3(key: number): Promise<v3.ClassDetails | undefined> {
    assert(this.isV3)
    return this._chain.getStorage(this.blockHash, 'Uniques', 'Class', key)
  }

  async getManyAsV3(keys: number[]): Promise<(v3.ClassDetails | undefined)[]> {
    assert(this.isV3)
...
export class UniquesInstanceMetadataOfStorage {
   ...
   *  Metadata of an asset instance.
   */
  async getAsV3(key1: number, key2: number): Promise<v3.InstanceMetadata | undefined> {
    assert(this.isV3)
    return this._chain.getStorage(this.blockHash, 'Uniques', 'InstanceMetadataOf', key1, key2)
  }

  async getManyAsV3(keys: [number, number][]): Promise<(v3.InstanceMetadata | undefined)[]> {
    assert(this.isV3)
